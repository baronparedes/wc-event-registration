...
export type ParsedRegistrationCsv = {
  headers: string[];
  rows: Record<string, string>[];
};

export type ParseRegistrationCsvResult =
  | { success: true; data: ParsedRegistrationCsv }
  | { success: false; error: string };

export type BulkRegistrationCsvRowInput = {
  member_id: string;
  registration_id?: string;
  answers: Record<string, unknown>;
};

export type BuildBulkRegistrationRowsResult = {
  rows: BulkRegistrationCsvRowInput[];
  errors: string[];
};

const REQUIRED_HEADERS = ['member_id'] as const;
...
export function parseRegistrationCsvText(csvText: string): ParseRegistrationCsvResult {
...
export function buildBulkRegistrationRowsFromCsv(
  csvRows: Record<string, string>[],
  fields: AdminEventField[],
  includedFieldKeys?: string[],
): BuildBulkRegistrationRowsResult {
  const rows: BulkRegistrationCsvRowInput[] = [];
  const errors: string[] = [];
  ...
  for (let index = 0; index < csvRows.length; index += 1) {
    ...
    const memberId = (csvRow.member_id ?? '').trim();
    if (memberId.length === 0) {
      errors.push(`Row ${rowNumber}: member_id is required.`);
      continue;
    }
    ...
    rows.push({
      member_id: memberId,
      registration_id: (csvRow.registration_id ?? '').trim() || undefined,
      answers,
    });
  }
