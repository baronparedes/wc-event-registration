...
import { useGetExcusedMembers } from '@/hooks/domain/members/queries/useGetExcusedMembers';
...
describe('useGetExcusedMembers', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('fetches excused member records successfully', async () => {
    const mockRecords = [
      {
        memberId: 'MEM-001',
    ...
    const { result } = renderHookWithClient(() => useGetExcusedMembers(2026, 8));
  ...
  it('throws default error message when error is missing in failed response', async () => {
    ...
    expect(result.current.error?.message).toBe('Failed to fetch excused members');
  });
});
