...
import { useMemberLookupMutation } from '@/hooks/domain/members';
import type { UserProfile } from '@/lib/domain/members';
...
export function FormSubmitPage() {
  ...
  const memberLookupMutation = useMemberLookupMutation();
  ...
  const handleMemberLookup = async () => {
    ...
    try {
      const result = await memberLookupMutation.mutateAsync({ query: memberId.trim() });
      if (result.success && result.member) {
        setMatchedMember(result.member);
        setActiveStep(2);
      } else {
        setMemberLookupError(result.error || 'Member not found');
      }
    } catch {
  ...
  const handleSubmitForm = async () => {
    ...
    try {
      await submitFormMutation.mutateAsync({
        form_slug: slug,
        member_id: matchedMember ? matchedMember.member_id : undefined,
        public_registrant_info: matchedMember ? undefined : publicInfo,
        responses,
        idempotency_key: `form-${slug}-${matchedMember?.member_id || publicInfo.email || 'anon'}-${Date.now()}`,
      });
            ...
            <div className="flex items-center justify-between border-b border-border pb-4">
              <div>
                ...
                <p className="text-xs text-muted mt-0.5">
                  Respondent:{' '}
                  {matchedMember
                    ? `${matchedMember.full_name} (${matchedMember.member_id})`
