...
import { useBulkUpsertPublicRegistrationsMutation } from '@/hooks/domain/public-registrations';
...
/** Bulk CSV upload UI for public registrations. */
export function BulkUploadPanel({
  ...
  const bulkUpsertMutation = useBulkUpsertPublicRegistrationsMutation();
  const [fileName, setFileName] = useState<string>('');
  const [preparedRows, setPreparedRows] = useState<BulkPublicRegistrationCsvRowInput[]>([]);
  ...
  function applyPreviewRowValidation(rows: Record<string, string>[], activeFieldKeys: string[]) {
    const builtRows = buildBulkPublicRegistrationRowsFromCsv(rows, fields, activeFieldKeys);
    ...
    const validator = buildBulkPublicRegistrationCsvRowsSchema(fields);
  ...
  async function handleFileChange(file: File | null) {
    ...
    const parsedCsv = parsePublicRegistrationCsvText(fileText);
  ...
  async function handleImport() {
    ...
    try {
      ...
      toast.success(
        `Imported ${result.imported_count} public registration(s) — ${result.created_count} created, ${result.updated_count} updated.`,
      );
      onClose();
    } catch (error) {
    ...
    <>
      {displayMode === 'overlay' && (
        <div className="border-b border-border px-6 py-4">
          ...
          <p className="mt-1 text-sm text-muted">
            Upload a CSV generated from the public registrations template to create or update public
      ...
      <div className="space-y-4 px-6 py-4">
        <div className="rounded-lg border border-border bg-background p-3">
          <label htmlFor="public-registrations-csv-file" className="text-sm font-medium text-text">
            CSV file
          </label>
          <input
            id="public-registrations-csv-file"
            type="file"
            accept=".csv,text/csv"
              ...
              <ListTable density="dense">
                <ListTableHead>
                  <ListTableHeaderRow variant="plain">
                    <ListTableHeaderCell className="w-44">First Name</ListTableHeaderCell>
                    <ListTableHeaderCell className="w-44">Last Name</ListTableHeaderCell>
                    <ListTableHeaderCell className="w-56">Email</ListTableHeaderCell>
                    <ListTableHeaderCell className="w-44">Nickname</ListTableHeaderCell>
                    <ListTableHeaderCell className="w-44">Phone</ListTableHeaderCell>
                ...
                <ListTableBody divider="default">
                  {parsedPreviewRows.map((row, index) => {
                    const rowKey = `${row.email ?? 'public'}-${index}`;

                    return (
                      <ListTableRow key={rowKey} hover="none">
                        {['first_name', 'last_name', 'email', 'nickname', 'phone'].map(
                          (fieldKey) => (
                            <ListTableCell key={fieldKey} className="align-top">
                              <input
                                id={`preview-${index}-${fieldKey}`}
                                type="text"
                                value={row[fieldKey] ?? ''}
                                onChange={(event) =>
                                  handleCellChange(index, fieldKey, event.target.value)
                                }
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="Import Public Registrations CSV"
        description={
          <div className="space-y-2">
            <p>
              This will create or update public registrations for the {preparedRows.length} row
              {preparedRows.length === 1 ? '' : 's'} included in this CSV.
            </p>
            <p>Continue only if you want to replace the existing answers for those rows.</p>
          </div>
        }
