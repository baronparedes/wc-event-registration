...
-- Drop previous 3-param signature to prevent ambiguous overload when calling with named parameters
drop function if exists public.get_service_dashboard_stats (integer, integer, date);
...
begin
  ...
  if p_start_date is not null and p_end_date is not null then
  ...
  elsif p_sunday_date is not null then
    ...
    raise exception 'Must provide a valid date range, year, or specific sunday date';
  end if;
...
revoke
execute on function public.get_service_dashboard_stats (integer, integer, date, date, date)
from
  public,
  anon,
  authenticated;
...
grant
execute on function public.get_service_dashboard_stats (integer, integer, date, date, date) to authenticated,
service_role;
