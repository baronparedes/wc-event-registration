...
-- Update get_commitment_dashboard_stats
...
begin
  ...
  with filtered_users as (
    ...
    and (
      -- If querying the whole month/year (p_time_slot is null) or specific time slot, ensure they have some form of matching commitment OR check-ins
      p_time_slot is null
      or exists (
        select 1
        from jsonb_each_text(u.metadata) as m(k, v)
        where m.k in ('first_sunday', 'second_sunday', 'third_sunday', 'fourth_sunday', 'fifth_sunday')
        and m.v ilike '%' || p_time_slot || '%'
      )
      or exists (
        select 1
        from public.user_commitment_history uch
        where uch.user_id = u.id
          and uch.effective_date >= p_start_date
          and uch.effective_date <= p_end_date
          and exists (
            select 1
            from jsonb_each_text(uch.metadata) as mh(k, v)
            where mh.k in ('first_sunday', 'second_sunday', 'third_sunday', 'fourth_sunday', 'fifth_sunday')
            and mh.v ilike '%' || p_time_slot || '%'
          )
      )
      or exists (
        select 1
        ...
        and sa.time_slot = p_time_slot
      )
    )
    and (
      p_role is null
      or u.user_category ilike '%' || p_role || '%'
    )
  ),
  excused_requests as (
    select distinct
    ...
    where r.user_id in (select user_id from filtered_users)
...
-- Update get_volunteer_attendance_log
...
-- Update get_member_service_attendance_stats
