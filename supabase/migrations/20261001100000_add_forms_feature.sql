...
create table public.forms (
  ...
  constraint forms_audience_allowed check (audience in ('members', 'public', 'members_and_public'))
);
...
create table public.form_fields (
  ...
  constraint form_fields_applicability_check check (field_applicability in ('all', 'member_only', 'public_only'))
);
...
create policy "public can read fields of published forms" on public.form_fields
  ...
  using (
    is_active = true and exists (
      select 1 from public.forms f where f.id = form_fields.form_id and f.status = 'published'
    )
  );
