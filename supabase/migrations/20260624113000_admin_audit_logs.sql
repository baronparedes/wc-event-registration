...
create table public.admin_audit_logs (
  ...
  constraint admin_audit_logs_resource_type_allowed check (
    resource_type in ('event', 'registration', 'export')
  )
);

create index admin_audit_logs_admin_created_idx on public.admin_audit_logs (admin_id, created_at desc);

create index admin_audit_logs_resource_created_idx on public.admin_audit_logs (resource_type, resource_id, created_at desc);

create index admin_audit_logs_created_idx on public.admin_audit_logs (created_at desc);

alter table public.admin_audit_logs enable row level security;

create policy "admins can read audit logs" on public.admin_audit_logs for
select
...
select
,
  insert on table public.admin_audit_logs to authenticated;
