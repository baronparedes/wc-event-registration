    ...
    select
      ...
      jsonb_strip_nulls(
        jsonb_build_object(
          ...
          metadata -> 'sr_pwd',
          'is_oic',
          coalesce(metadata -> 'is_oic', metadata -> 'isoic'),
          'first_sunday',
          coalesce(
...
set
  metadata = (
    (
      coalesce(n.metadata, '{}'::jsonb) - 'role' - 'category' - 'sr_pwd' - 'is_oic' - 'isoic' - 'availability' - '1st_sunday' - '2nd_sunday' - '3rd_sunday' - '4th_sunday' - '5th_sunday' - 'first_sunday' - 'second_sunday' - 'third_sunday' - 'fourth_sunday' - 'fifth_sunday'
