...
-- Update RLS policy on users table to allow users to read their own record matched by email
...
-- Update get_member_event_history function to allow users to fetch their own event history
...
with
    ...
    (
        select
            ...
            'registration_answers', coalesce(registration_answers.answers, '[]'::jsonb),
            'attendance_answers', coalesce(attendance_answers.answers, '[]'::jsonb),
            'slot_records', coalesce(slot_records.records, '[]'::jsonb)
          ) as item,
...
select coalesce(jsonb_agg(item order by sort_key desc nulls last), '[]'::jsonb)
    from history
  );
end;
$$;
