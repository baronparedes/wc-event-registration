...
async function fetchInChunks<T>(params: {
  tableName: string;
  selectSql: string;
  foreignKeyColumn: string;
  ids: string[];
  buildQueryFn: (idsChunk: string[]) => PromiseLike<{ data: T[] | null; error: unknown }>;
}): Promise<T[]> {
  const { tableName, selectSql, foreignKeyColumn, ids, buildQueryFn } = params;
  ...
  for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
    ...
    const { data, error } = await buildQueryFn(chunk);
    if (error) {
      const chunkError = new Error(
        `Chunk fetch failed for ${tableName} (${foreignKeyColumn}) at ${start}-${end}`,
      ) as ChunkedFetchError;
      chunkError.tableName = tableName;
      chunkError.chunkIndex = chunkIndex + 1;
      chunkError.chunkSize = chunk.length;
      chunkError.idRange = `${start}-${end}`;
      chunkError.selectSql = selectSql;
      chunkError.foreignKeyColumn = foreignKeyColumn;
      chunkError.originalError = error;
      throw chunkError;
    }
  ...
  try {
    ...
    const checkInsQuery =
      allRegistrationRefIds.length > 0
        ? fetchInChunks<CheckInRow>({
            tableName: 'attendance_check_ins',
            selectSql: 'registration_id, first_checked_in_at',
            foreignKeyColumn: 'registration_id',
            ids: allRegistrationRefIds,
            buildQueryFn: (registrationIdChunk) =>
              adminClient
    ...
    const attendanceAnswersQuery =
      registrationIds.length > 0
        ? fetchInChunks<AttendanceAnswerRow>({
            tableName: 'attendance_answers',
            selectSql:
    ...
    const registrationAnswersQuery =
      registrationIds.length > 0
        ? fetchInChunks<RegistrationAnswerRow>({
    ...
    let checkInsResult;
    let publicCheckInsResult;
    let attendanceAnswersResult;
    let publicAttendanceAnswersResult;
    let registrationAnswersResult;
    let publicRegistrationAnswersResult;
    ...
    try {
      ...
      const chunkError = error as Partial<ChunkedFetchError>;
      const isChunkError =
        typeof chunkError?.tableName === 'string' &&
        typeof chunkError?.chunkIndex === 'number' &&
        typeof chunkError?.chunkSize === 'number' &&
        typeof chunkError?.idRange === 'string';

      logListAttendeesError('load_attendee_details_chunk_failure', error, {
        event_id,
        registration_count: registrationIds.length,
        public_registration_count: publicRegistrationIds.length,
        ...(isChunkError
          ? {
              tableName: chunkError.tableName,
              chunkIndex: chunkError.chunkIndex,
              chunkSize: chunkError.chunkSize,
              idRange: chunkError.idRange,
              chunkError: chunkError.originalError,
            }
          : {}),
      });
