...
export function createGetExcusedMembersTool({ client, requestId }: ToolContext) {
  ...
  return tool({
    ...
    execute: async ({ role, timeframe }) => {
      ...
      const queryMonth = targetSundays[0];
      ...
      const targetDates = new Set(targetSundays.map((date) => date.toISOString().slice(0, 10)));
      ...
      if (userIds.length === 0) {
        return {
          ...
          sunday_breakdown: targetSundays.map((date) => ({
      ...
      const dateKeys = targetSundays.map((date) => date.toISOString().slice(0, 10));
